...
⭐ 🚨 **嚴禁機械式等距 / 等高（AI lazy mode 警告）**：
   實拍紙張一定有透視、邊緣彎曲、拍攝角度差。即使版面看起來規整，
   行間距 normalized 後**不可能完全相等**、各題 h 也**不可能完全相同**。
   若你發現自己輸出的多題 Δy 變異 < 5%、或多題 h 數值相同，
   代表你**沒有真的看每一題的紙面位置**，是用第一題測量值做等差級數外推的 lazy mode。
   → 偵測到這個信號 → 重新針對**每一題**逐個量測，每題都要看著該題真實墨跡邊緣決定 y、h。
