...
- For visible=true questions with question_context/group_context, output answerBbox that frames the FULL QUESTION CONTEXT so a teacher can see the entire question at a glance:
  ...
  - For fill_blank sub-questions (questionId has 3+ segments, e.g. "1-2-1", "1-2-2", "1-2-3"): each sub-question maps to ONE specific blank box. answerBbox must frame the student's handwritten answer inside that blank.
    ...
    ORDINAL RULE (MANDATORY when blankOrdinal is present): When the spec includes blankOrdinal and blankTotal, this line contains MULTIPLE blanks. You MUST count the PRINTED blank markers (parentheses, boxes, underlines) from LEFT to RIGHT on the same printed line, and frame the Nth blank marker (where N = blankOrdinal). Example: if blankOrdinal=1, blankTotal=2, the line has 2 printed blank markers — frame the FIRST (leftmost) one. If blankOrdinal=2, frame the SECOND one.
    ...
    BBOX FROM PRINTED MARKS (MANDATORY for fill_blank sub-questions): Determine answerBbox position and size from the PRINTED blank markers (parentheses, boxes, underlines), NOT from the handwritten content inside. Set x to the left edge of the printed marker (e.g. the opening parenthesis "("), and w to span the full printed blank area (e.g. from "(" to ")"). Handwriting varies in size, darkness, and position — it MUST NOT influence bbox x, y, or w. The printed markers are fixed landmarks; use them as your sole reference for positioning.
    Examples:
