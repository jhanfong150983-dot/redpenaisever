...
- For visible=true questions, output answerBbox per the type-specific rules below. **每個 type 的規則決定 bbox 該含什麼/不含什麼**，沒有「預設要含題幹脈絡」這回事 — 一切以 per-type rule 為準。
  ...
  - For fill_blank 子題（questionId 含 3+ segments，如 "1-2-1"）：每個子題對應題幹中的一個空白標記（( )、□、___），學生在標記內寫值（含單位則一併寫）。answerBbox 緊框該空白標記區（含學生手寫），不含題幹。不可漂移到相鄰子題的空白、不可跨行。
    ...
    **不可**用「鄰近印刷單字」當 bbox 起點的代理。
    ...
    🚨 BBOX 邊界 ≠ 學生手寫位置（極重要）：
    bbox 邊界由**印刷標記**（底線 _____ / 括號 ( ) / 方框 □）決定，**不是**由學生手寫位置決定。
    學生手寫應該**在 bbox 內部**，不是 bbox 的邊界。

    - bbox 左邊 = 印刷標記最左端的像素（底線左端 / "(" 字符的左邊 / "□" 的左邊）
    - bbox 右邊 = 印刷標記最右端的像素（底線右端 / ")" 字符的右邊 / "□" 的右邊）
                  OR 學生手寫超出印刷標記的右端（容忍 overflow），**取較右者**
    ...
    【Bug 1：底線型同行多空（英文常見）】
    - ❌ 看到「Mom is _____ in the _____」，把 "is" / "the" 當 anchor 推位置
      → bbox 偏左 → 切到「cookin」尾巴
    - ❌ 把 "in" 當第二底線的 anchor → bbox 起點落在 "in" → 還沒到第二底線就開始畫

    【Bug 2：括號型 + 末尾單位（數學常見）— 你最該避免的 bug】
    - ❌ 看到「(    150    ) 元」，把 bbox 框得只包學生手寫「150」的位置
      → 印刷括號 "(" 在 bbox 外面 → 裁切結果只看到「)元」或「50    )元」
      → 切掉學生答案的左半部
    - ❌ 看到「150」就以「150」為中心向外擴 → 抓不到 "(" 和 ")"
    - ✅ 正確：先目視找到 "(" 和 ")" 的像素位置，bbox 從 "(" 包到 ")"，
       「150」在 bbox 內部即可

    🔍 視覺自我檢查（每題一定要做）：
    bbox 邊緣**第一眼看到的應該是印刷標記的字元**（"(", ")", "□", "_"），不該是：
    - 印刷字母（i / t / n / 是 / 的）→ anchor 選錯了
    - 印刷單位（元 / 公分 / 個）→ bbox 太右
    - 學生手寫的數字/字母邊緣 → bbox 太緊（漏框印刷括號）
    若 bbox 內看不到完整的「( )」或「___」標記 → bbox 邊界錯了，重畫。

    範例：
    - 括號型（無單位）：「請填(  )」→ 框 ( ) 兩個字符 + 內部空間 + 學生手寫
    - 括號型（含單位）：「(    150    ) 元」→ ✅ 框 ( 150 )；❌ 不可只框「150」也不可框到「元」
    - 方框型：「2½ □ (4.73 □ 2.73)」→ 框該 □ 格本身（含學生寫的 ×/+/− 等運算符）
    - 底線型：「___ 公尺」→ 框 ___ 底線區 + 學生手寫的數字（含超出底線的尾巴），
       但「公尺」在外面
    - 同行多空（底線型）：「Mom is _____ in the _____.」blankTotal=2
       ✅ ordinal=1 → bbox 緊框第 1 個 _____ + 手寫 cooking
       ✅ ordinal=2 → bbox 緊框第 2 個 _____ + 手寫 kitchen
       ❌ bbox 偏左切到「cookin」尾
    - 同行多空（括號型）：「(  3  ) + (  5  ) = (  8  )」blankTotal=3
       ✅ ordinal=1 → bbox 框第 1 個 ( ) 含 "3"
       ✅ ordinal=2 → bbox 框第 2 個 ( ) 含 "5"
       ✅ ordinal=3 → bbox 框第 3 個 ( ) 含 "8"
       ❌ bbox 框到「+」「=」
