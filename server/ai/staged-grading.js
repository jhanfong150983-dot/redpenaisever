...
- For visible=true questions, output answerBbox per the type-specific rules below. **每個 type 的規則決定 bbox 該含什麼/不含什麼**，沒有「預設要含題幹脈絡」這回事 — 一切以 per-type rule 為準。
  ...
  - For fill_blank 子題（questionId 含 3+ segments，如 "1-2-1"）：每個子題對應題幹中的一個空白標記（( )、□、___），學生在標記內寫值（含單位則一併寫）。answerBbox 緊框該空白標記區（含學生手寫），不含題幹。不可漂移到相鄰子題的空白、不可跨行。
    ...
    ⚠️ 此格式下，「手寫位置」≠「答案區位置」
    ⚠️ 不要試圖去視覺找 "(" 像素 — 印刷括號太淡你抓不到，會把 bbox 貼著手寫導致左半被切掉
    策略：先目測手寫最左端，**再向左推約 3-5% 頁寬**到推估的 \`(\` 位置
    理由：印刷的 \`(\` 雖然視覺上不顯著，但它**一定**在手寫左側
    範例計算：
      - 手寫「410」最左端在 x=0.55
      - bbox.x = 0.55 - 0.04 = 0.51（涵蓋 \`(\`）
      - bbox.x + w = 手寫最右端 + 0.01~0.02（涵蓋 \`)\`，但不含「元」）

    【box 方框型】
    視覺結構：類似 parens，但 □ 邊界較明顯可目視
    策略：可目視 □ 邊界，bbox 緊框 □ 含內部手寫（含學生寫的 ×/+/− 等運算符）
    ...
    2. parens 時，我有把 bbox.x 向左推到 \`(\` 位置嗎？
       檢查：bbox.x 與手寫最左端的距離應該 ≥ 0.03（頁寬）
    3. underline 時，bbox.x 應該 = 手寫最左端（不需多推）
    4. 框出的 bbox 內**不該**含到題幹文字、相鄰空格的內容、或末尾的單位

    ❌ 常見 bug：

    【Bug 1：parens 沒向左推 → 切到左半（最該避免）】
    - 看到「(    150    ) 元」，bbox 直接貼著手寫「150」
      → 印刷括號 "(" 在 bbox 外面 → 裁切結果只看到「)元」或「50    )元」
    - ✅ 正確：手寫「150」最左端在 x=0.50，bbox.x = 0.50 - 0.04 = 0.46

    【Bug 2：parens 推太多 → 含到前一題】
    - bbox.x 向左推超過 0.06，會跨進左邊鄰題的空格
    - ✅ 控制在 0.03-0.05 之間

    【Bug 3：underline 同行多空把鄰字當 anchor】
    - 看到「Mom is _____ in the _____」，把 "is" / "the" 當推位置的依據
      → bbox 起點落在 "is" / "in" → 還沒到底線就開始畫，切到鄰字
    - ✅ 正確：直接從第 N 條底線最左端的像素起算（= 手寫最左端）

    範例：
    - 括號型（無單位）：「請填(  3  )」blankFormat=parens
       手寫「3」在 x=0.40 → bbox.x = 0.36，w 涵蓋到 ")" 右側
    - 括號型（含單位）：「(    150    ) 元」blankFormat=parens
       手寫「150」最左端 x=0.50 → bbox.x = 0.46；w 到 ")" 右側但不含「元」
    ...
    - 同行多空（括號型）：「(  3  ) + (  5  ) = (  8  )」blankTotal=3
       ✅ ordinal=N → 找第 N 個手寫數字最左端，向左推 0.04
       ❌ bbox.x 直接貼著數字 → 切到 \`(\`
