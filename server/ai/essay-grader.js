...
只輸出 JSON：{"columns":["最右直行的文字","下一直行的文字"]}`
}

/** 抄寫回覆 → 該行文字（一次只送一行，取第一筆） */
export function parseEssayTranscribeColumn(text) {
  if (!text) return null
  try {
...
【請輸出以下回饋】
1. typos：文中的錯別字（學生寫成另一個字的「別字」、用錯的同音/形近字）。wrong=學生寫的字詞、correct=正確字詞、context=包含該錯字的原文片段（逐字照抄、6~12 字）。沒有就給空陣列。不要把口語用詞、標點問題列進來。
