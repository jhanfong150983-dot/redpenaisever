...
## 六、落地結果與拍板（2026-08-15）

`Phase 0b-7 確定性優先` 已實作於 `server/ai/deterministic-compare.js` + `staged-grading.js`，
kill switch `CODE_FIRST_ENABLED='false'`。

**全庫回放 43,935 格**：判得動 27,475（62.5%）、與現行分數相同 26,808、差異 160。

| 差異 | 筆數 | 歸因 | 處置 |
|---|---|---|---|
| code 修正 AI 判錯 | 47 | 跨記號等價誤殺 10、選項放水 7、舊事故 0 分 26、代數放水 3… | ✅ 要的 |
| AI 用了答案卷沒寫的標準 | 83 | 答案卷寫「94.2」AI 要求「94.2 平方公分」；英語卷未開 punctuationCheck 仍為句點扣分 | ⬅ 見拍板 |
| accessor 看圖修正讀值錯位 | 13 | 讀值「8%」實為「10」，AI 核對影像後改判 | ⬅ 見拍板 |

### user 拍板

1. **以答案卷為準**。答案卷是老師寫的 SSoT，AI 不得自行加碼學科慣例。
   AI 自行加碼正是「同一份答案卷在不同批次有不同標準」的來源；要單位/標點就寫進答案卷，
   那樣老師也改得動。
2. **接受失去「accessor 看圖修正讀值」**。讀值錯位屬於 read 階段的問題，應該在那裡解；
   靠 accessor 事後救本來就不穩定（同批救到、別批沒有）。

### 待辦（未做）

- 322 題缺 `questionCategory` → 永遠走不到任何 code 路徑（影響面最大）
- 查表制覆蓋率不到 1%（僅國語 short_answer + rubricsDimensions）
- 4–6 月「理由說對卻 0 分」舊事故共約 100 格，分數至今仍是錯的
