...
2. 逐格（兩段式）：格外擴 25%（含題號）、遮格內手寫，±5mm 找 NCC 峰。pass 1 量各格位移 → 中位位移 ≤ 3mm 就用它把 H 拉正
   （SIFT/RANSAC 的 H 跨平台可差 1〜2mm：同一份卷本機 0.9mm、Cloud Run 2.4mm）→ pass 2 重量，最佳位移 ≤ 2.5mm ＝ 該格一致。
   一致率 ≥ 0.75 才用（同版面 ≥ 0.81；另一版答案卷多數 ≤ 0.6；實測 數學 12/12、國語 12/12、另版 0/12 全退、別科 25/25 退）。
3. 吸附：投影邊 2.5mm 內有學生印刷線就吸上去（掃描進紙漸進歪斜由此吸收）；沒線的格用該格 NCC 位移。
4. SIFT 對學生合併圖「逐頁段」計算（整張 1200×5100 一次算會吃 1.5GB → Cloud Run OOM）；Cloud Run 規格 2GiB / 1 CPU / concurrency 1。
